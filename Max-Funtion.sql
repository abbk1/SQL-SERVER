...
select MAX(SaleDate) [Maximum Date] from Sales

select MAX(PaymentMethod) [Maximum Amount] from Sales

-- Grouping By ProductID and getting maximun quantity and sum of sales
select productid, MAX(Quantity) [Max-Qty], SUM(totalamount) [Total Sale] from Sales group by ProductID
